...
describe('App autosave integration', () => {
  it('retries a retained failed save from the toolbar without applying the edit twice', async () => {
    ...
    const inputs = positionedSheet('sheet-inputs', 'Inputs', { x: 120, y: 80 });
    const writeCells = vi.fn()
      .mockRejectedValueOnce(new Error('backend unavailable'))
      .mockReturnValueOnce(retrySave.promise.then((response) => ({ sheets: [response] })));
    const apiClient = autosaveClient({
      writeCells,
      // Recovery must inspect the persisted state before a failed operation can be retried.
      loadSheet: vi.fn().mockResolvedValue(inputs),
    });

    render(
      <App
        initialWorkbook={workbookWithSheets([inputs])}
        apiClient={apiClient}
      />,
    );
