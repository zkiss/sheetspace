...
function cloneIntent(intent: WorkbookPersistenceIntent): WorkbookPersistenceIntent {
  switch (intent.kind) {
    ...
    case 'write-number-formats': return { ...intent, writes: intent.writes.map((write) => ({ ...write, properties: { ...write.properties, numberFormat: write.properties.numberFormat && { ...write.properties.numberFormat } } })) };
