...
class FormulaParser {
  ...
  private readExpression(): { ok: true; value: FormulaExpression } | { ok: false; error: FormulaErrorCode } {
    this.skipWhitespace();
    ...
    if (identifier) {
      ...
      this.skipWhitespace();
      if (this.peek() === '(') {
        return this.readFunctionCall(identifier, identifierStart);
      }
